...
export function ClearableNumberInput({
  ...
  const handleFocus = () => {
    ...
    setInputValue(Number.isFinite(value) && value !== 0 ? value.toString() : '')
  }
...
export function CurrencyInput({
  ...
  return <ClearableNumberInput value={value} onChange={onChange} step={step} showCurrency={true} />
}
...
export function LabeledCurrency({
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <ClearableNumberInput value={value} onChange={onChange} step={step} className="w-24" showCurrency={true} />
    </div>
  )
}
...
export function LabeledNumber({
  ...
  return (
    <div className="flex items-center gap-2">
      <div className="text-[11px] text-slate-600">{label}</div>
      <ClearableNumberInput value={value} onChange={onChange} step={step} className="w-24" />
    </div>
  )
}

export function DualCell({ left, right }: { left: React.ReactNode; right: React.ReactNode }) {
  return (
...
export function ValueText({ children, className = "" }: { children: React.ReactNode; className?: string }) {
