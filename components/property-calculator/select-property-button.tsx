...
export default function SelectPropertyButton({
  ...
  const getFolderProperties = (folderName: string) => {
    return mockSavedProperties.filter(prop => prop.folder === folderName && prop.type === propertyType)
  }
    ...
    <>
      <DropdownMenu open={isOpen} onOpenChange={setIsOpen} modal={false}>
        <DropdownMenuTrigger asChild>
          <Button variant="outline" className="w-full h-8 text-xs font-semibold text-slate-700 bg-transparent">
        ...
        >
          ...
          >
            ...
            <span>Create New {propertyType} Entry</span>
          </DropdownMenuItem>
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="text-xs text-slate-500">
                            {property.type} - {formatPrice(property.purchasePrice)}
                          </div>
                        </div>
                      </div>
                    ))}
                    {folderProperties.length === 0 && (
                      <div className="px-3 py-2 text-slate-500 pl-8 text-sm">
