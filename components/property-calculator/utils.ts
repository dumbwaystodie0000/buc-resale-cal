...
export const fmtNumber = (n: number, options?: {
  minimumFractionDigits?: number
  maximumFractionDigits?: number
  locale?: string
}) => {
...
export const fmtPercent = (n: number, dp = 1) => `${Number.isFinite(n) ? n.toFixed(dp) : "0.0"}%`

export const fmtRate = (n: number) => (Number.isInteger(n) ? `${n}%` : `${n.toFixed(2)}%`)

export function isFieldNotApplicable(property: Property, field: keyof Property, mode: Mode) {
  // Fields always N/A for BUC, regardless of mode
  const commonBucNAFields = ["minorRenovation", "furnitureFittings", "propertyTax", "maintenanceFee", "monthlyRental"]
  ...
  if (field === "agentCommission" && mode === "own" && property.type === "Resale") {
    return true
  }
